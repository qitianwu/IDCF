...
class GCMCModel(nn.Module):
	...
	def forward(self, x,  edge_UI = None, edge_IU = None, edge_UI_l = None, edge_IU_l = None, sample_graph = False):
		user_id = x[:, 0]
		item_id = x[:, 1]
...
class IRMC_GC_Model(nn.Module):
	...
	def forward(self, x, history, history_len, edge_UI = None, edge_IU = None, sample_graph = False, mode='INTER'):
		user_id = x[:, 0]
		item_id = x[:, 1]
